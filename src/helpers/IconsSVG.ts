import AddResumeIcon from '../assets/svg/icons/addResume.svg';
import AddVideoIcon from '../assets/svg/icons/addVideo.svg';
import AppleIcon from '../assets/svg/icons/apple.svg';
import ArrowDownIcon from '../assets/svg/icons/arrowDown.svg';
import ArchiveIcon from '../assets/svg/icons/archive.svg';
import BackIcon from '../assets/svg/icons/back.svg';
import BackBottomIcon from '../assets/svg/icons/backBottom.svg';
import BackBottomLightIcon from '../assets/svg/icons/backBottomLight.svg';
import BackLightIcon from '../assets/svg/icons/backLight.svg';
import BasketIcon from '../assets/svg/icons/basket.svg';
import ChatIcon from '../assets/svg/icons/chat.svg';
import ChatActiveIcon from '../assets/svg/icons/chatActive.svg';
import CheckIcon from '../assets/svg/icons/check.svg';
import CircleOutlineIcon from '../assets/svg/icons/circleOutline.svg';
import CloseIcon from '../assets/svg/icons/close.svg';
import EditIcon from '../assets/svg/icons/edit.svg';
import FeedbackIcon from '../assets/svg/icons/feedback.svg';
import FeedbackActiveIcon from '../assets/svg/icons/feedbackActive.svg';
import FiltersIcon from '../assets/svg/icons/filters.svg';
import FiltersDarkIcon from '../assets/svg/icons/filtersDark.svg';
import FlagIcon from '../assets/svg/icons/flag.svg';
import FlagRedIcon from '../assets/svg/icons/flag_red.svg';
import GeoIcon from '../assets/svg/icons/geo_gray.svg';
import GeoDarkIcon from '../assets/svg/icons/geo_dark.svg';
import GoogleIcon from '../assets/svg/icons/google.svg';
import LikeIcon from '../assets/svg/icons/like.svg';
import LikeActiveIcon from '../assets/svg/icons/likeActive.svg';
import LikeActiveTabIcon from '../assets/svg/icons/likeActiveTab.svg';
import LineGradientIcon from '../assets/svg/icons/lineActive.svg';
import LineGrayIcon from '../assets/svg/icons/lineGray.svg';
import NextIcon from '../assets/svg/icons/next.svg';
import NextPlayIcon from '../assets/svg/icons/nextSpeed.svg';
import PlayIcon from '../assets/svg/icons/play.svg';
import Play2Icon from '../assets/svg/icons/playII.svg';
import PlusIcon from '../assets/svg/icons/plus.svg';
import PrevPlayIcon from '../assets/svg/icons/prevSpeed.svg';
import ProfileIcon from '../assets/svg/icons/profile.svg';
import ProfileActiveIcon from '../assets/svg/icons/profileActiveTab.svg';
import RoundGradientIcon from '../assets/svg/icons/round_gradient.svg';
import RoundGradientOutlineIcon from '../assets/svg/icons/round_gradient_outline.svg';
import SearchIcon from '../assets/svg/icons/search.svg';
import SearchActiveIcon from '../assets/svg/icons/searchActive.svg';
import SendIcon from '../assets/svg/icons/send.svg';
import ShareIcon from '../assets/svg/icons/share.svg';
import CaptureIcon from '../assets/svg/icons/shoot.svg';
import SplashIcon from '../assets/svg/icons/splash.svg';
import StarIcon from '../assets/svg/icons/star.svg';
import StopIcon from '../assets/svg/icons/stop.svg';
import TimeIcon from '../assets/svg/icons/timer.svg';
import TurnIcon from '../assets/svg/icons/turn.svg';
import HeartActiveFullIcon from '../assets/svg/icons/heartActiveFull.svg';
import ErrorPageIcon from '../assets/svg/screen/error.svg';
import Background_Chat from '../assets/svg/background/chat_background.svg';
import Background_Ellipse from '../assets/svg/background/ellipse3.svg';
import Background_TutorialBottomRight from '../assets/svg/background/tutorial_bottom-right.svg';
import Background_TutorialLeftTOP from '../assets/svg/background/tutorial_left-top.svg';

export const IconsSvg = {
  AddResumeIcon,
  AddVideoIcon,
  AppleIcon,
  ArrowDownIcon,
  ArchiveIcon,
  BackIcon,
  BackBottomIcon,
  BackBottomLightIcon,
  BackLightIcon,
  BasketIcon,
  ChatIcon,
  ChatActiveIcon,
  CheckIcon,
  CircleOutlineIcon,
  CloseIcon,
  EditIcon,
  FeedbackIcon,
  FeedbackActiveIcon,
  FiltersIcon,
  FiltersDarkIcon,
  FlagIcon,
  FlagRedIcon,
  GeoIcon,
  GeoDarkIcon,
  GoogleIcon,
  LikeIcon,
  LikeActiveIcon,
  LikeActiveTabIcon,
  LineGradientIcon,
  LineGrayIcon,
  NextIcon,
  NextPlayIcon,
  PlayIcon,
  Play2Icon,
  PlusIcon,
  HeartActiveFullIcon,
  PrevPlayIcon,
  ProfileIcon,
  ProfileActiveIcon,
  RoundGradientIcon,
  RoundGradientOutlineIcon,
  SearchIcon,
  SearchActiveIcon,
  SendIcon,
  ShareIcon,
  CaptureIcon,
  SplashIcon,
  StarIcon,
  StopIcon,
  TimeIcon,
  TurnIcon,
  ErrorPageIcon,
  Background_Chat,
  Background_Ellipse,
  Background_TutorialBottomRight,
  Background_TutorialLeftTOP,
};
